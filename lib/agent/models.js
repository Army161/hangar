...
function recommendLocal(freeVramGb) {
  if (!Number.isFinite(freeVramGb)) return { tier: 'unknown', freeGb: null, advice: 'VRAM not detected — pick a model manually.' };
  const freeGb = Number(freeVramGb.toFixed(1));
  if (freeVramGb >= 7) return { tier: 'large', freeGb, maxParams: '8-9B', advice: `${freeGb} GB free — 8B class models fit comfortably.` };
  if (freeVramGb >= 4) return { tier: 'medium', freeGb, maxParams: '3-4B', advice: `${freeGb} GB free — stay at 3-4B; 8B will OOM.` };
  return { tier: 'small', freeGb, maxParams: '1-2B', advice: `${freeGb} GB free — use a 1-2B model, or a cloud provider with your own key.` };
}

/**
 * Will this model load in the VRAM available right now?
 *
 * Weights are not the whole cost — the KV cache, context, and the runtime's own
 * overhead all sit alongside them, and a model that exactly equals free VRAM
 * will not load. HEADROOM is the margin, set from the case PLAN §4 records:
 * a 6.6 GB model OOMing reproducibly below ~5 GB free means the real cost of a
 * 6.6 GB file is meaningfully above 6.6 GB.
 *
 * Returns null when the question does not apply — cloud models have no local
 * footprint, and an undetected GPU is not a reason to call something unfit.
 */
const VRAM_HEADROOM = 1.15;

function fitsVram(model, freeGb) {
  if (!model || !model.sizeGb) return null;      // cloud, or size unknown
  if (!Number.isFinite(freeGb)) return null;     // no reading — do not guess
  return model.sizeGb * VRAM_HEADROOM <= freeGb;
}

/**
 * The model to select by default.
 *
 * Largest model that actually fits, because bigger is better when it runs at
 * all. Falls back to the smallest local model when nothing fits — that is more
 * useful than defaulting to something guaranteed to OOM — and to the first
 * cloud model with a key when there is no local option.
 */
function pickDefault(models, freeGb) {
  const local = models.filter((m) => m.sizeGb);
  const fitting = local.filter((m) => fitsVram(m, freeGb));

  if (fitting.length) return fitting.sort((a, b) => b.sizeGb - a.sizeGb)[0];
  if (local.length) return local.sort((a, b) => a.sizeGb - b.sizeGb)[0];
...
module.exports = {
  PROVIDERS, CURATED, VRAM_HEADROOM,
  recommendLocal, fitsVram, pickDefault,
  discoverOllama, discoverOpenAICompatible, discoverAll,
